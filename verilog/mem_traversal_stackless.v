...
module mem_traversal_stackless(
  input power, clk, rst,
  ...
  always @(posedge clk or negedge rst) begin
    if (!rst) begin
      ...
      state <= SYS_READ_INIT;
      mem_addr <= {`memory_addr_width{1'b0}};
      trav_B <= `NIL;
      trav_P <= {`noun_width{1'b0}};
      mem_execute <= 1'b0;
      gc_ready <= 1'b0;
      debug_sig <= 0;
        ...
        SYS_FUNC_EXECUTE: begin
          case (state)
            ...
            SYS_EXECUTE_READ_ADDR: begin
              if (mem_ready) begin
                if (trav_B != `NIL) mem_addr <= trav_B[`memory_addr_width - 1:0];
                debug_sig <= 10;
          ...
          case (state)
            ...
            SYS_READ_GC_WAIT: begin
              ...
              if (!gc && gc_ready) begin
                ...
                trav_P <= read_data1[`memory_addr_width - 1:0];
        ...
        SYS_FUNC_TRAVERSE: begin
          case (state)
              ...
              if (mem_data[`large_atom_bit]) begin
                ...
                case (mem_tag[1:0])
                  `CELL_CELL: begin
                    ...
                    end else if (mem_tag[3:2] == 2'b10) begin
                      write_return_sys_func <= SYS_FUNC_TRAVERSE;
                      write_return_state <= SYS_TRAVERSE_PUSH;
                      trav_B <= hed;
                      trav_P <= trav_B;
                    ...
                    end else if (mem_tag[3:2] == 2'b11) begin
            ...
            SYS_TRAVERSE_PUSH: begin
              mem_addr <= trav_P[`memory_addr_width - 1:0];
              sys_func <= SYS_FUNC_READ;
              state <= SYS_READ_INIT;
            end
